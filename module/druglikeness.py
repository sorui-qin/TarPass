...
LastEditTime: 2026-09-07 14:59:53
Description:
"""
...
class DruglikenessCalculator:
    ...
    def calculate_all(self, mol: Chem.Mol) -> dict[str, float | int]:
        if mol is None:
            return {}
    ...
    def heteroatom_ratio(self) -> float:
        ...
        return (
            self.properties.get("heteroatoms", 0) / heavy_atoms
            if heavy_atoms > 0
            else np.nan
        )
...
def calc_le(args: tuple[Mol, float]) -> dict:
    ...
    """
    pose, score = args
    return {"LE_heavyatom": le_heavyatom(pose, score), "LE_mw": le_mw(pose, score)}
...
def molvol(mol) -> float:
    ...
    if mol.GetNumConformers() == 0:
        state = EmbedMolecule(
            mol_Hs, randomSeed=42, useRandomCoords=True, maxAttempts=100
        )
        if state != 0:
            mol_Hs = obmol2rdkit(LigPrep(mol_Hs).obmol_conf(mol_Hs))
    if mol_Hs.GetNumConformers() == 0:
        project_logger.warning(
            "Failed to generate 3D conformation, return molecular volume with NaN."
        )
        return np.nan
