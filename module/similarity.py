...
def get_similarity(
    mols1: list[Mol], mols2: list[Mol] | None, limit=10000, seed=0, use_ref: bool = True
) -> float:
...
class Similarity:
    def __init__(
        self, smis: list[str], ref_smis: list[str] | None = None, limit=10000, seed=0
    ):
        self.smis = smis
    ...
    def circle(self, thershold=0.75) -> float:
        ...
        measure = NCircles(
            vectorizer=vectorizer, sim_mat_func=sim_mat_func, threshold=thershold
        )
    ...
    def scaffold_similarity(self, use_ref: bool = True) -> float:
        ...
        scaff2 = (
            scaffolds(sample(self.ref, n=self.limit, fixed_seed=self.seed))
            if self.ref
            else None
        )
