...
def get_parser():
    ...
    parser.add_argument(
        "--epoch", "-e", type=int, default=100, help="epoch that SMILES was sampled"
    )
    parser.add_argument(
        "--n_jobs", "-n", type=int, default=40, help="number of processes to use"
    )

    parser.add_argument(
        "--input_file", "-i", type=str, default="./", help="path to sampled files"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--sample_id", "-si", type=int, default="0", help="Index of sampling"
    )
    return parser
...
def set_canonical(n_jobs, output, file_set):
    ...
    with Pool(n_jobs) as pool:
        ...
        dataset_cano = [
            x for x in pool.imap_unordered(smile_canonical_p, dataset) if x is not None
        ]
    dataset = []
...
def CollectRingSystems(smis, n_jobs=1):
    ...
    with Pool(n_jobs) as pool:
        ...
        rings = [
            x for x in pool.imap_unordered(GetRingSystems_p, smis) if x is not None
        ]
