...
class DissimilarityBasedMeasure(Measure):
    def __init__(self, vectorizer=None, sim_mat_func=None, *args, **kargs):
...
class Diversity(DissimilarityBasedMeasure):
    ...
    def measure(self, mols=[], is_vec=False):
        ...
        return 1.0 - np.sum(avg_dists) / len(avg_dists)
...
def sumbot_map(args):
    vec, vecs, sim_mat_func = args
    dists = 1.0 - sim_mat_func([vec], vecs)[0]
...
from module.chemmeasure.cal_ifg_atom import CollectFG
from module.chemmeasure.cal_ringsystem import CollectRingSystems
...
class NFragment(ReferenceBasedMeasure):
    def __init__(self, frag=None):
        super().__init__()
        if frag == "FG":
            self.collect_fn = CollectFG
        elif frag == "RS":
            self.collect_fn = CollectRingSystems
...
class NCircles(Measure):
    def __init__(
        self, vectorizer=None, sim_mat_func=None, threshold=0.75, *args, **kargs
    ):
        super().__init__(*args, **kargs)
        self.vectorizer = vectorizer
        self.sim_mat_func = sim_mat_func
    ...
    def measure(self, mols=[], is_vec=False, n_chunk=64):
        ...
        for i in range(3):
            ...
            random.shuffle(vecs)
        vecs = get_circles((vecs, self.sim_mat_func, self.t), silent=False)
