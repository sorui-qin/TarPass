...
from dock.dock_class import BatchDock, SingleDock
...
def breakpoint_check(result_pkl: Path, total_lens: int) -> int:
    if result_pkl.exists():
        project_logger.info("Detected previous docking results.")
        results = read_pkl(result_pkl)
        if isinstance(results, list):
            latest_idx = len(results)
            if latest_idx > total_lens:
                raise ValueError(
                    f"Index {latest_idx} exceeds the total number of ligands {total_lens}."
                )
            project_logger.info(
                f"Docking will start from {latest_idx + 1} of {total_lens} molecules."
            )
            return latest_idx
    return 0
...
def save_results(pkl_path, index, mol, pose, score, mode):
    ...
    if mode == "dock":
        append_pkl(
            pkl_path, {"index": index, "mol": mol, "pose": pose, "docking score": score}
        )
...
def setup_arguments(parser: argparse.ArgumentParser):
    ...
    group1.add_argument(
        ...
        "--mode",
        type=str,
        choices=["dock", "score_only", "all"],
        default="all",
        help="docking mode.",
    )
    group1.add_argument(
        "--verbose", type=int, help="verbosity level of the docking process."
    )
    ...
    group1.add_argument(
        "--config",
        type=str,
        default=f"{ROOT}/dock/config/gnina_dock.yml",
    ...
    group1.add_argument(
        ...
        type=str,
        choices=["gnina", "vina"],
        help="docking method to use (default: gnina).",
    )
    ...
    group2.add_argument(
        "--reset",
        action="store_true",
        help="reset the original 3D conformation if available.",
    )
    group2.add_argument(
        "--in_single",
        action="store_true",
        default=False,
        help="docking in single mode.",
    )
    return parser
...
def execute(args):
    ...
    for target in TARGETS:
        ...
        if args.mode == "all":
            if conformation_check(mols):
                ...
                project_logger.warning(
                    f"Conformations are not detected in {target}, switching to `dock` mode."
                )
                modes = ["dock"]
        else:
            modes = [args.mode]
        ...
        for mode in modes:
            ...
            if args.reset:
                result_pkl = (
                    target_dir / f"results/{args.method}-reset_docking_results.pkl"
                )
            else:
                result_pkl = (
                    target_dir / f"results/{args.method}-{mode}_docking_results.pkl"
                )
            ...
            if args.in_single or latest_idx != 0:
                for i, mol in tqdm(
                    enumerate(latest := mols[latest_idx:]),
                    desc=f"Docking with {target}",
                    total=len(latest),
                ):
                ...
                project_logger.info(
                    f"Batch docking with {target}, total {total_lens}..."
                )
                ...
                if len(results) != total_lens:
                    raise ValueError(
                        f"Batch docking results length {len(results)} does not match total ligands {total_lens}."
                    )

            project_logger.info(
                f"Docking in {target} completed. Results saved in {result_pkl}."
            )
