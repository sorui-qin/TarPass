...
class BaseDockTask:
    ...
    def __init__(
        self, ligand: str, target: str, mode: Literal["dock", "score_only"] = "dock"
    ):
        if not isinstance(ligand, str):
            raise ValueError(
                "Invalid ligand input, please provide a filename or pdbqt string."
            )
...
class GninaDock(BaseDockTask):
    ...
    def __init__(
        ...
        try:
            ...
            raise FileNotFoundError(
                f"No config files {self.target}.txt found in `<INSTALL_PATH>/dock/paras`"
            )
    ...
    def run(
        self, seed=0, exhaust=8, n_poses=1, verbose=0, in_batch=False
    ) -> tuple[Mol, float] | list[tuple[Mol, float]]:
        ...
        with temp_manager(".sdf") as tmp_file:
            command = [
                ...
                str(exhaust),
                "--cnn",
                "fast",  # using fast cnn for no GPU decive
            ]
