...
def execute(args):
    ...
    for target in TARGETS:
        ...
        if mode == "dock":
            ...
            if not docked_pkl:
                raise ValueError(
                    f"Docking results not found in {target_dir}, please running `tarpass dock`."
                )
            docked_results = read_pkl(docked_pkl)
            if len(docked_results) < len(mols):
                raise ValueError(
                    "Docking results are incomplete, please rerun `tarpass dock`."
                )
        ...
        if store_pkl.exists():
            ...
            if len(processed) == len(poses):
                project_logger.info(
                    f"Interaction analysis already completed for {len(poses)} molecules."
                )
                continue
