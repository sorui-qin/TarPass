...
def combined_pdb_complex(pdb: str | Path, ligand: Mol, output_pdb: str):
    ...
    try:
        ...
        project_logger.warning(
            f"Failed to uncharge ligand: {e}. Proceeding with original ligand."
        )
...
def analyze_plip(pdb: str | Path) -> dict[str, list]:
    ...
    analyzer.analyze()
    interactions = analyzer.interaction_sets[
        "UNL:Z:1"
    ]  # UNL:Z:1 is default if combined with RDKit
...
def get_interactions(interactions: PLInteraction) -> defaultdict[str, list]:
    ...
    inters["H-Bond"].extend(
        inters["H-Bond acceptor"] + inters["H-Bond donor"]
    )  # Combine H-Bond acceptor and donor
...
def match_interactions(detect_inters: dict, key_inters: dict) -> dict:
    ...
    result["matched_rate"] = (
        successful_matches / total_checks if total_checks > 0 else 0
    )
...
def interactions(poses: list[Mol], empty_pdb: Path, key_inters: dict) -> list[dict]:
    ...
    if conformation_check(poses):
        ...
        analyze_func = partial(
            analyze_tmppdb, empty_pdb=empty_pdb, key_inters=key_inters
        )
        return process_map(
            analyze_func, poses, desc="Analyzing interactions", chunksize=10
        )
    else:
        return []
