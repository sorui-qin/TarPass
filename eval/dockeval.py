...
class DockEval:
    ...
    def __init__(self, poses: list[Mol], scores: list[float], target: str):
        ...
        if self.lens != len(scores):
            raise ValueError(
                f"Length of poses ({len(poses)}) and scores ({len(scores)}) must be equal."
            )
        self.poses = poses
        self.scores = scores
        self.target = target
        self.pdb_path, self.lig_path = self._get_target()
        self.pdb_rdmol, self.lig_mol = (
            read_pdb_rdmol(self.pdb_path),
            read_sdf(self.lig_path),
        )
    ...
    def centriod_shift(self):
        ...
        return process_map(
            ...
            self.poses,
            chunksize=100,
            desc="Calculating centroid shift",
        )
    ...
    def ligand_efficiency(self):
        ...
        return process_map(
            calc_le,
            zip(self.poses, self.scores),
            chunksize=100,
            desc="Calculating ligand efficiency",
            total=self.lens,
        )
    ...
    def esp_sim(self):
        ...
        return process_map(
            ...
            args,
            chunksize=100,
            desc="Calculating ESP similarity",
        )
...
def clash_check(mol_pred, mol_cond):
    ...
    return {
        "no_clashes": check_intermolecular_distance(mol_pred, mol_cond=mol_cond)[
            "results"
        ]["no_clashes"]
    }
...
def extract_results(
    results: list[dict], mode: Literal["dock", "score_only"]
) -> tuple[list, list]:
    """Extract docking results from the results directory."""
    mapping = {"dock": ("pose", "docking score"), "score_only": ("mol", "vina score")}
    p, s = mapping[mode]
...
def dock_eval(target_dir: Path) -> list[dict]:
    ...
    mols = [
        res["mol"] for res in dock_results
    ]  # Load original molecules from docking results
    ...
    if score_results:
        project_logger.info(
            "Initial molecules are 3D, checking scoring results of initial conformation..."
        )
        score_eval_results = _extract_and_eval(results=score_results, mode="score_only")
    else:
        project_logger.info(
            f"Initial molecules are not 3D, skipping scoring results for {target}."
        )
        score_eval_results = []
    ...
    for idx, (smiles, dock_res, score_res) in enumerate(
        itertools.zip_longest(
            smis, dock_eval_results, score_eval_results, fillvalue=None
        )
    ):
        info_di = {
            "index": idx,
...
def dockeval_execute(args):
    ...
    for target in TARGETS:
        ...
        results_dir = target_dir / "results"
        eval_output = results_dir / "dock_eval_results.json"
