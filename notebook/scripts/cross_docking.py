...
def cross_dock_eval(b_dir, target_a):
    ...
    b_mols = [
        i["mol"] for i in read_pkl(b_dir / "results/gnina-dock_docking_results.pkl")
    ]
...
def main(args):
    ...
    project_logger.info("Cross-docking completed.")


parser = argparse.ArgumentParser(
    description="Execute and analyze cross-docking for two specific targets."
)
parser.add_argument(
    "--path",
    "-p",
    type=str,
    required=True,
    help="Path to the docking results directory.",
)
parser.add_argument(
    "--target_a",
    "-a",
    type=str,
    required=True,
    help="First target name for cross-docking.",
)
parser.add_argument(
    "--target_b",
    "-b",
    type=str,
    required=True,
    help="Second target name for cross-docking.",
)
