...
def centriod_distance(mol_pred: Mol, mol_ref: Mol):
    ...
    return np.linalg.norm(np.array(centroid_pred) - np.array(centroid_cond))
...
class LigPrep:
    ...
    def ligprep(self, polaronly=True, correctforph=True, PH=7.4) -> Chem.Mol:
        ...
        if not self.mol.GetNumConformers():  # Check 3D conformation
            try:  # RDKit pipeline
                ...
                if MMFFOptimizeMolecule(
                    p_mol, maxIters=200
                ):  # If fail to optimize, use Open Babel
        ...
        if prep_mol is None and backup:
            project_logger.warning(
                f"Protonation failed in {self.mol.GetProp('_Name')}, using original conformation instead."
            )
            prep_mol = backup
        return prep_mol
    ...
    def get_pdbqt(self, **kwargs):
        ...
        if prep_mol:
            ...
            project_logger.warning(
                f"Ligand preparation failed in {self.mol.GetProp('_Name')}."
            )
            return None
