...
from utils.constant import TMP, Path
...
def read_pdb_rdmol(pdb_file: str | Path, sanitize=False, removeHs=False):
    mol = Chem.MolFromPDBFile(str(pdb_file), sanitize=sanitize, removeHs=removeHs)
    return mol
...
def read_sdf(sdf_file: str | Path, sanitize=False, removeHs=False):
    mols = [
        ...
        for mol in Chem.SDMolSupplier(
            str(sdf_file), sanitize=sanitize, removeHs=removeHs
        )
        if mol
    ]
...
def read_smi(smi_file: str | Path, delimiter="", sanitize=False):
    supp = Chem.SmilesMolSupplier(
        str(smi_file), delimiter=delimiter, sanitize=sanitize, titleLine=False
    )
...
def temp_manager(suffix: str, output_dir=TMP, auto_remove=True):
    ...
    with tempfile.NamedTemporaryFile(
        suffix=suffix, dir=output_dir, delete=False
    ) as tmp:
        tmp_file = tmp.name
    try:
        yield tmp_file
