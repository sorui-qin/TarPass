...
def conformation_check(mols: list[Chem.Mol]) -> bool:
    ...
    if any(confs) + any(confs) == 0:
        ...
        raise RuntimeError(
            "Some molecules have not 3D conformation. Please check the input molecules."
        )
    return True
...
class Preprocess:
    ...
    def valid(self) -> list[Chem.Mol]:
        if self.format == "sdf":
            valids = [
                mol for idx, mol in enumerate(self.readins) if sanitize_valid(mol, idx)
            ]
        elif self.format == "smi":
            valids = [
                mol
                for idx, smi in enumerate(self.readins)
                if (mol := smiles_valid(smi, idx))
            ]
    ...
    def unique(self) -> defaultdict[str, list[Chem.Mol]]:
        ...
        project_logger.info(
            f"Unique SMILES: {len(unique_di.items())} out of {len(mols)}"
        )
        return unique_di


def read_in(
    target_dir, num_thres=1000, isomers=False
    ...
    """
    # NOTE: Modify here when the read-in path is different
    read_dir = Path(target_dir)  # /'generated'
    ...
    if isomers and format == "sdf":
        project_logger.info("Checking 3D Conformation...")
        for smi, mols in processed_li:
            proc_smis.append(smi)
            proc_mols.extend(check_duplicate3D(mols))
        project_logger.info(
            f"3D Conformation checked, {len(proc_mols)} unique 3D conformations found."
        )

    if len(proc_smis) < num_thres:
        project_logger.warning(
            f"Not enough unique molecules found. Read with {len(proc_smis)} unique molecules, but expected {num_thres}."
        )
    else:
        project_logger.info(
            f"Read in procession is done, top {num_thres} unique molecules are selected."
        )
        ...
        series = pd.Series(
            {
                ...
                "unique_3d": round(len(unique_3d) / len(unique_mol), 4)
                if unique_mol
                else 0,
            }
        )
