...
def find_dockpkl(
    results_dir: str | Path, mode: Literal["dock", "origin", "score_only"]
) -> Path:
    if (m := mode) == "origin":
        m = "score_only"
    patterns = ["gnina", "vina"]
    ...
    raise FileNotFoundError(
        f"No docking results file found in '{results_dir}' for mode '{mode}'.\
...
def read_eval(
    results_dir: str | Path, mode: Literal["dock", "mole"]
    ...
    if fn is None or fname is None:
        raise ValueError(
            f"Invalid mode '{mode}'. Expected one of {list(EVAL_MAP.keys())}."
        )
    eval_path = Path(results_dir) / fname
    if not eval_path.exists():
        raise FileNotFoundError(
            f"Evaluation file '{eval_path}' does not exist, please rerun `tarpass {mode}eval."
        )
    return fn(eval_path)
