...
def mann_whitney_u(
    data1, data2, alternative: Literal["two-sided", "less", "greater"] = "two-sided"
) -> tuple[bool, float]:
    ...
    Args:
        ...
        alternative (str): Defines the alternative hypothesis. Options are 'two-sided', 'less', or 'greater'.
    """
    _, p_value = stats.mannwhitneyu(data1, data2, alternative=alternative)
...
def dunn(
    *data_groups, control, p_adjust: Literal["bonferroni", "fdr_bh"] = "fdr_bh"
...
def multiple_correction(
    p_values: np.ndarray, method: Literal["bonferroni", "fdr_bh"] = "fdr_bh"
) -> tuple[np.ndarray, np.ndarray]:
