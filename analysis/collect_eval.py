...
class CollectEval:
    ...
    def __init__(self, results_dir: str | Path):
        ...
        self.score_dicts = (
            [res["score"] for res in self.dock_eval] if self.confs else []
        )
    ...
    def _read_eval(self) -> tuple[list[dict], pd.DataFrame]:
        ...
        if len(dock_eval) != len(mole_eval):
            raise ValueError(
                f"Docking and Molecule evaluation results length mismatch for {self.target}."
            )
        return dock_eval, mole_eval

    def split_dock(
        self, mode: Literal["dock", "score"], split_key: str = "matched_rate"
    ) -> tuple[dict, dict]:
        ...
        if split_key not in dicts[0]:
            raise ValueError(
                f"Split key '{split_key}' not found in dictionary. Available keys: {list(dicts[0].keys())}"
            )
        ...
        return (
            pd.DataFrame(numericals).to_dict(orient="list"),
...
class MoleculesData:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if index < 0 or index >= num_molecules:
            raise IndexError(
                f"Index out of range: {index}. Valid range is 0 to {num_molecules - 1}."
            )
        return {
            "Mol": {
            ...
            "Dock": {
                ...
                "interactions": {
                    k: v[index] for k, v in self.Dock.interactions.items()
                },
            },
            "Score": {
                "numerical": {k: v[index] for k, v in self.Score.numericals.items()},
                "interactions": {
                    k: v[index] for k, v in self.Score.interactions.items()
                },
            }
            if self.Score
            else None,
...
def collect_eval(results_dir: Path) -> MoleculesData:
    ...
    Mol = MolInfo(
        rdmol=rdmol_list, pose=pose_list, smiles=smiles_list, inchikey=inchikey_list
    )
    ...
    Score = (
        StructInfo(numericals=nums_s, interactions=inters_s)
        if all([nums_s, inters_s])
        else None
    )

    descriptors, structural, alerts = collect.split_mole()
    Prop = PropInfo(descriptors=descriptors, structural=structural, alerts=alerts)

    return MoleculesData(Mol=Mol, Dock=Dock, Score=Score, Prop=Prop)
...
def collect_eval_all(
    ...
    for target in tqdm(TARGETS, desc="Collecting eval results from targets"):
        ...
        if not target_dir.exists():
            ...
            continue
        results[target] = (
            collect_eval(target_dir / "results")
            if not readable
            else collect_readable(target_dir / "results")
        )
