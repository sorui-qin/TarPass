...
from analysis.analysis_class import (
    DATA_DIR,
    MolDisAnalysis,
    PLIAnalysis,
    PLIAnalysisScore,
...
def setup_arguments(parser: argparse.ArgumentParser):
    parser.add_argument(
        ...
        "-o",
        type=str,
        help="Directory of the output file where the results will be saved. \
...
def execute(args):
    ...
    for target in tqdm(TARGETS, desc="Analysis targets"):
        ...
        assert isinstance(test_target, MoleculesData), (
            f"Invalid data for target {target}"
        )
